...
{
    public class EOCRequestLogRepository
    {
        OrganizationServiceProxy _organizationService;

        public EOCRequestLogRepository(OrganizationServiceProxy org)
        {
        ...
        public Guid Create(string url, bool success, long elapsedTime, string request, string response, string error)
        {
            try
            {
                Entity entity = new Entity("amx_eocrequestlog");
            ...
            {
                throw new Exception("EOCLogRepository Create: " + ex.Message);
            }
        }
    }
}