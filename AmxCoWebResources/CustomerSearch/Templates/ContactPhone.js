function doSimpleSearch() {
    ...
    function fetchcustomer(searchText) {
        var fetchIndividual = "<fetch distinct='false' no-lock='true' mapping='logical' output-format='xml-platform' version='1.0'>"
			...
			+ "<filter type='or'>"
			+ "<condition attribute='mobilephone' value='" + searchText + "' operator='eq'/>"
			+ "<condition attribute='telephone1' value='" + searchText + "' operator='eq'/>"
			+ "<condition attribute='telephone2' value='" + searchText + "' operator='eq'/>"
			+ "<condition attribute='telephone3' value='" + searchText + "' operator='eq'/>"
			+ "</filter>"
			+ "</entity>"
			+ "</fetch>";
