...
function onChangeDocumentNumber() {
    ...
    if (DocumentNumber != null) {
        if ((DocumentType == DNI) && (DocumentNumber.length != DocIdLengthDNI)) {
        ...
        else if ((DocumentType == CE) && (DocumentNumber.length != DocIdLengthCE)) {
            DocIdValidationAlert = FormatString(DocIdValidationAlert, DocumentTypeText, DocIdLengthDNI);
            alert(DocIdValidationAlert);
            Xrm.Page.getAttribute("amxperu_documentid").setValue(null);
        }
        else if ((DocumentType == RUC) && (DocumentNumber.length != DocIdLengthRUC)) {
            DocIdValidationAlert = FormatString(DocIdValidationAlert, DocumentTypeText, DocIdLengthDNI);
            alert(DocIdValidationAlert);
            Xrm.Page.getAttribute("amxperu_documentid").setValue(null);
        }
        else if (Xrm.Page.getAttribute("amxperu_documentid").getValue() != null) {
            checkWithContactDocumentID(DocumentNumber);
        }
    }
}
function checkWithContactDocumentID(DocumentNumber) {
     {
